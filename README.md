...
一款支持 **Godot Engine 4.x** 与 **Pygame-CE** 运行的跨平台动态生成迷宫游戏。支持主界面**模式选择**（🌟 **自由模式**与 🏆 **闯关模式**）、**调试选关按钮**（🎯 主界面与游戏右侧边栏直观按钮，支持一键直跳第一大关 1~10 阶与 🏰 **第二大关 1~10 阶地牢**）、**🧭 一键自动寻路**（点击右侧按钮或按 `H` 键生成并高亮从起点大树到终点小屋的最优路线光轨）、**双大关自然演进**（🌲 **第一大关：绿野森林 1-10阶** DFS 算法 vs 🏰 **第二大关：暗黑狼穴地牢 1-10阶** 高密转弯 Prim + 伪环路高难算法）、**四大场景专属 FC 8-bit 背景音乐**（🎮 **主界面温馨曲**、🌟 **自由模式明快探险曲**、🏆 **闯关模式热血英雄曲**、🏰 **狼穴地牢诡异阴森 FC 恶魔城风格曲**）与**主界面独立音效/BGM音量调节**、小红帽与大灰狼童话主题、大树起点与小屋终点、**右侧独立记分牌与不遮挡地图的状态栏**、各阶难度得分计算（**破纪录分数翻倍**）、**0.01s 精确计时器**与**20 关大满贯全通总用时及最高分持久化保存**。
...
1. **🌟 自由模式 (Free Mode)**
   - 无限玩，随意选择第一大关 (1~10阶) 或 第二大关 (1~10阶) 进行练习。
   - 随时按 `R` 键重新随机生成，支持随时按数字键或选关按钮切关。
   - 适合练习路线、熟悉地图视口与挑战单关最佳纪录。

2. **🏆 闯关模式 (Challenge Mode)**
   - 从 **第一大关 1 阶连续挑战至第二大关 10 阶 (共 20 关)** 大满贯！
   - 每通过一关自动计算关卡积分并进入下一阶。
   - 20 关全通关后进行大满贯结算，记录并保存 **20 关全通总用时** 与 **全通总得分**！
